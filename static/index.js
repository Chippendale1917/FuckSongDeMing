...
function gameTime() {
    _gameTimeNum--;
    if (_gameTimeNum <= 0) {
        GameTimeLayer.innerHTML = '&nbsp;&nbsp;&nbsp;&nbsp;狗德明死绝了';
        gameOver();
...
function showGameScoreLayer() {
    ...
    document.getElementById('GameScoreLayer-bast').innerHTML = '最高击杀&nbsp;&nbsp;' + bast;
    l.style.display = 'block';
}
...
function shareText(score) {
    ...
    SubmitResults();
    if (score <= 49) return '德明很生气';
    if (score <= 99) return '同雪，你楚来一下';
    if (score <= 149) return '你泽洋做思埠不太合④啊同雪';
    if (score <= 199) return '你觉得你特对呗';
    return '恁就是宋德明吧？';
}
